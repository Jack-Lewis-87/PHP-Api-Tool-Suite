...
include_once(dirname(__DIR__)."/List/ListEndpoint.php");         //Call Specifc//Incomplete//

class ListPost extends ListEndpoint {
     ...
     * @var array
     */
    protected $description = "Create a list. Return value will include a create_time if the list is not new (just modified). To add/remove users from a list use the job call.";
    ...
    private $cli_params__method = [
        ...
        "primary" => ["primary","Primary: Set to 1 to make the list a primary list, 0 to make the list non-primary."],
        "vars" => ["vars","Vars: Enter a JSON object or prefix each var name with \"var_\"."],
        "public_name" => ["public_name","Public Name: Assign a public name for a list that will be displayed to subscribers (i.e. on an optout page)."],
        "query" => ["query","Smart List Query: Use the API Query format to create the underlying query criteria for a Smart List. This needs to be improved."],
        "type" => ["type","Type: Set to \"smart\" to make the list a Smart List, or \"normal\" for a normal list"],
        // "" => [],
    ];
