...
$environment = "https://api.sailthru.com";
$last_error;
$exit = 0;
...
if (file_exists($cred_file)) {
	...
	if (filesize($cred_file) < 900) {
        echo "Your Key and Secret file wasn't completed or has been currupted. Sorry, we need to create it again.\n\n";
	} else {
		$exit = 1;
...
if ($edit_mode == null) {
	...
	fwrite($creds, "*  This file is generated by Setup.php\n");
	fwrite($creds, "*  Any changes to the underlying structure should be instigated there.\n");
	fwrite($creds, "*/\n");
	...
	fwrite($creds, "\tprotected static \$apiEnvironmentDefault = \"".$environment."\";\n");	
	...
	fwrite($creds, "\t\t\t\"environment\" => \"https://api.sailthru.com\",\n");
	fwrite($creds, "\t\t],\n");
	fwrite($creds, "\t\t".$id." => [\n");
	fwrite($creds, "\t\t\t\"key\" => \"".$key."\",\n");
	fwrite($creds, "\t\t\t\"secret\" => \"".$secret."\",\n");
	fwrite($creds, "\t\t\t\"name\" => \"".$name."\",\n");
	fwrite($creds, "\t\t\t\"environment\" => \"".$environment."\",\n");
	fwrite($creds, "\t\t],\n");
	...
	while (($line = fgets($creds)) !== FALSE) {
		...
		if (trim($line) == $startDefaultsLine && $is_edit_defaults) {
			...
			$file_contents .= "\tprotected static \$apiEnvironmentDefault = \"".$environment."\";\n";
			continue;
		}
		if (trim($line) == $defaultDelimiterLine) {
			if (isset($is_add_new) && $is_add_new) {
				...
				$file_contents .= "\t\t\t\"environment\" => \"".$environment."\",\n";
				$file_contents .= "\t\t],\n";
			}
...
//I should probably just overload this function wherever it exists, rather than have a local version. Meh
